    ...
    (data, index) =>
      index < 10 && (
        <tr className={styles.table__data} key={data.device_id}>
          ...
          <td>
            <Center>
              {data.attributes.department?.data?.attributes.department_name}{data.attributes.production?.data?.attributes.name}
            </Center>
          </td>
          <td>
          ...
          <td>
            {Fecha(data.attributes.maintenance?.data?.attributes.next_maintenance)}</td>
          <td>
            {data.attributes.maintenance?.data?.attributes.maintenance_type_next}
          </td>
          <td>
            <div className={styles.icons}>
              ...
              color="dark" 
              variant="transparent"
              onClick={()=> {setOpenedMaint(true); setDeviceToMaint(data)}}
              >
                <IconTool size={18} />
              </ActionIcon>
              <ActionIcon onClick={() => {
      ...
      >
        <Postpone maintToPostpone={{ ...maintToPostpone}} closeModal={closeModal} />
      </Modal>
      )}
